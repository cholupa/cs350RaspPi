...
class CWMachine(StateMachine):
    ...
    def on_enter_dot(self):
        ...
        ## use LED blink function to turn on LED
        self.redLight.blink(on_time=0.500, n=1, background=False)
    ...
    def on_exit_dot(self):
        # Red light forced off using LED.off()
        self.redLight.off()
    ...
    def on_enter_dash(self):
        # Blue light comes on for 1500ms by LED.blink()
        self.blueLight.blink(on_time=1.500, n=1, background=False)
    ...
    def on_exit_dash(self):
        # Blue light forced off via LED.off()
        self.blueLight.off()
    ...
    ## PAUSE events controlled by sleep() function for specified durations
    ...
    def toggleMessage(self):
        ...
        # check active message if message1 or message2 then set to opposite
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        ## Dot or dash?
                        #if char is a dot, then exit dash and enter dot
                        #if char is a dash, then exit dot and enter dash 
                        if(x=='.'):
                            self.on_exit_dash()
                            self.on_enter_dot()
                        elif(x=='-'):
                            self.on_exit_dot()
                            self.on_enter_dash()
                        #after entering either state, enter the character pause
                        self.on_enter_dotDashPause()
                        # If we are still sending process a dotDashPause event

                        # if the spot isn't at end of the char line, increment the counter
                        if(morseCounter < lenMorse):
                            morseCounter += 1
                        #if at the end of the char line, reset the counter for next morse char line
                        # at end of morse line exit morse pause and enter letter pause for next letter
                        elif(morseCounter == lenMorse):
                            self.on_exit_dotDashPause()
                            self.on_enter_letterPause()
                            morseCounter = 1

                    # If we are still sending process a letterPause event
                    # check counter to length of the word, increment if less than the length
                    if(wordCounter < lenWord):
                        wordCounter += 1
                    # if at length, exit letter pause event and enter word pause event
                    elif(wordCounter == lenWord):
                ...
                #check the word counter to the num of words, if less then increment counter
                if(wordsCounter < lenWords):
                    wordsCounter += 1
                #if equal the num of words, then exit the pause event and reset the counter
                if(wordsCounter == lenWords):
...
#set the callback of the button when pressed to the processButton function
