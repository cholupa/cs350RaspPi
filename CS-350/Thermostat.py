...
class TemperatureMachine(StateMachine):
    ...
    def processTempIncButton(self):
        # increment set point, call updateLights() to check light status  
        self.setPoint += 1
        self.updateLights()
        if(DEBUG):
    ...
    def processTempDecButton(self):
        # decrease set point, call to updateLights() checking light status
        self.setPoint -= 1
        self.updateLights()
        if(DEBUG):
    ...
    def setupSerialOutput(self):
        #output string delimited by commas (current state, current temp, set point)
        output = f"Current State: {self.current_state_value},Current Temp: {floor(self.getFahrenheit())},Set to: {self.setPoint}"
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
            # after 5 seconds display current temp value
                lcd_line_2 ="Current T: "+  str(floor(self.getFahrenheit()))
    
                altCounter = altCounter + 1
            else:
            #alternate to the current set point and state value
                lcd_line_2 = "Set: " + str(self.setPoint) + " " + self.current_state_value
            ...
            if((counter % 30) == 0):
            # every 30 seconds write to the thermostat server displaying (current state, current temp, set point)
