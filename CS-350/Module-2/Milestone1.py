...
while repeat:
    try:
        # loop to cycle maximum then sleep for 1 tenth of a second
        for inc in range(0,101,5):
            pwm18.ChangeDutyCycle(inc)
            time.sleep(0.1)
        # loop from max to 0, decrement by 5, sleep also 1 tenth of a second
        for inc in range(100,-1,-5):
            pwm18.ChangeDutyCycle(inc)
            time.sleep(0.05)

    except KeyboardInterrupt:
        # Stop the PWM instance on GPIO line 18 by ctrl+c
